...
{
    public class AnnualSallesReports
    {
        private readonly IWebHostEnvironment webHostEnvironment;
        private readonly TheTopDatabaseContext context;
        public AnnualSallesReports(IWebHostEnvironment webHostEnvironment, TheTopDatabaseContext context)
        {
        ...
        public void ReportBody()
        {
            ...
            for (int i = 1 ; i<=12 ;i++ )
            {
                ...
                pdfTable.AddCell(pdfCell);
                    ...
                    pdfCell = new PdfPCell(new Phrase(context.Sales.Where(x=>x.Date.Month == i && x.Date.Year == DateTime.Now.Year).Sum(x=> (x.Design.Cost.ToString()==null? 0 : x.Design.Cost)).ToString(), fontStyle));
                    pdfCell.HorizontalAlignment = Element.ALIGN_CENTER;
                    pdfCell.VerticalAlignment = Element.ALIGN_MIDDLE;
                if (i % 2 != 0)
                    pdfCell.BackgroundColor = BaseColor.White;
